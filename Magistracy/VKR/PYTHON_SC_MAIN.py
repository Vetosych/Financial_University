...
model_name = 'Модель PD для Магистерской'
...
VOL_cutoff        = 0.60 # верхний уровень отсечения волатильности
...
CORREL_cutoff     = 0.45 # верхний уровень отсечения по парной корреляции для переменной

## Настройки для вывода валидационных статистик

ROCAUC_n_bucket  = 20 # кол-во бакетов в графике построения ROCAUC
PREDICT_n_bucket = 20 # кол-во бакетов в графике построения PREDICT
...
filepath_or_buffer_1 = r'application_train(model) PD.csv' # путь к файлу 1
...
use_restart_file = True # флаг использования файла с данными для рестарта: True/False
filepath_restart = r'C:\Users\Виталий\Desktop\Python_actual\Магистерская\Model # 2022-05-04 23_08_20\Model # 2022-05-04 23_08_20 # 3_backup.xlsx' # путь к файлу с данными для рестарта

use_restart_binning = True        # флаг использования посчитанного бинирования: True/False
...
change_types_data = False  # флаг преобразования типов данных на основе их наименований: True/False
...
stop_list_attr = [] # стоп-лист переменных: не используются в модели
...
model_type_1  = True # 'Логистическая регрессия "LogisticRegressionCV"'
...
if use_oracle == True: ## Загрузка из хранилища Oracle
    ...
    if use_file_1 == True: ## загрузка 1 файла с переменными
        df1 = download_csv_file(filepath_or_buffer_1, mysep = ',', myindex_col = 0)
...
for i, j in long_list_short.iterrows():
    if j.cnt == 1 and j.iv_gap_pred > 0.35 and j.iv_gap_next > 0.35:
